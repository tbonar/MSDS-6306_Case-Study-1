...
```{r}
# count each occurence of each state
breweriesMapData = count(filter(breweries,!grepl('DC', State)), State)

# Rename "n" column to "Total"
colnames(breweriesMapData)[2] = "Total"
breweriesMapData

# Create data frame with State name and abbreviation
stateLookup = data.frame(abbreviation = state.abb, State = state.name)
